...
def is_valid_input_data(data: list[dict]) -> tuple[bool, list]:
    ...
    for d in data:
        ...
        is_data_valid = is_valid_cid(d["cid"]) and is_valid_location(
            d["location"]
        )
...
def are_candidated_sheet_cells_empty(
    data: list[dict], sheet_data: dict
) -> tuple[bool, list]:
...
def fill_in_google_sheet(data: list[dict], sheet_data: dict) -> bool:
    ...
    for d in data:
        ...
        mac_chr = chr(65 + headers["MAC"])
        ip_chr = chr(65 + headers["IP"])
        ...
        # append mac data who is to be filled in
        mac_data = {
            "range": f"{table}!{mac_chr}{row_index}",
            "values": [[d["mac"]]],
        }
        batch_update_data.append(mac_data)

        # append ip data who is to be filled in
        ip_data = {
            "range": f"{table}!{ip_chr}{row_index}",
            "values": [[d["ip"]]],
        }
        batch_update_data.append(ip_data)
...
def update_cert_lab_google_sheet(data: list[dict]) -> dict:
    ...
    empty, non_empty_list = are_candidated_sheet_cells_empty(data, sheet_data)
